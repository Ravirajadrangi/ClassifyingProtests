...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type is 'rnn':
            h, h_cache = rnn_forward(words, h0, Wx, Wh, b)
        else:
            h, h_cache = lstm_forward(words, h0, Wx, Wh, b)

        temp_out, temp_cache = temporal_affine_forward(h, W_vocab, b_vocab)
        loss, dout = temporal_softmax_loss(temp_out, captions_out, mask, verbose=False)
        ...
        grads['W_embed'] = word_embedding_backward(dout, words_cache)
    ...
    def sample(self, features, max_length=30):
        ...
        next_token = np.ones((N,1), dtype=np.int32) * self._start

        for t in range(max_length):
            words, words_cache = word_embedding_forward(next_token, W_embed)
            words = np.squeeze(words)
            if self.cell_type is 'rnn':
                h, _ = rnn_step_forward(words, h, Wx, Wh, b)
            else:
                h, c, _ = lstm_step_forward(words, h, c, Wx, Wh, b)

            scores = h.dot(W_vocab) + b_vocab
            captions[np.arange(N),t] = np.argmax(scores, axis=1)
            next_token = np.argmax(scores,axis=1)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return captions
